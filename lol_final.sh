...
# lol_final.sh - Versión final: DOOM-APP / Vega AI (completo)
set -euo pipefail
APP_DIR="$HOME/DOOM-APP"
PUBLIC_DIR="$APP_DIR/public"

echo "🚀 lol_final.sh — iniciando setup en $APP_DIR (final)"
...
ENCRYPT_DB_PASSWORD=
ENV
echo "🔑 .env creado. Edita .env para colocar tu GEMINI_API_KEY y opcionales."
else
echo "ℹ .env ya existe — no se sobrescribe"
fi

# --- db.json inicial (estructura extendida) ---
cat > db.json <<'JSON'
{
  "users": [],
  "servers": [],
...
# --- db.js (lowdb helpers + optional encryption) ---
cat > db.js <<'JS'
...
export async function save() {
  await db.write()
  // optional encryption: if ENCRYPT_DB_PASSWORD set, overwrite db.json encrypted (simple)
...
export async function ensureDecrypted() {
  // if file is encrypted format, decrypt into memory (used on startup)
  try {
    const raw = fs.readFileSync(FILE,'utf8')
    const parsed = JSON.parse(raw)
    if (parsed && parsed.__encrypted && process.env.ENCRYPT_DB_PASSWORD) {
      const bytes = CryptoJS.AES.decrypt(parsed.data, process.env.ENCRYPT_DB_PASSWORD)
      const dec = bytes.toString(CryptoJS.enc.Utf8)
      fs.writeFileSync(FILE, dec)
      await db.read()
    }
  } catch(e){}
}
...
# --- vega_ai.js (Gemini wrapper + simple TTS via gtts) ---
cat > vega_ai.js <<'JS'
...
export async function ttsSave(text){
  const out = path.join('public','tts_'+Date.now()+'.mp3')
...
server.listen(PORT, ()=> console.log('✅ Vega AI server moviendo en http://localhost:'+PORT))
JS

# --- public files: index.html, style.css, app.js with features (contacts, servers, channels, messages, edit/delete/react/pin, notifications, local cache, jitsi link) ---
cat > "$PUBLIC_DIR/index.html" <<'HTML'
<!doctype html>
<html lang="es">
<head>
<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Vega AI — DOOM APP (final)</title>
  ...
  <main class="main">
    ...
    <section class="chat-area">
      ...
      <div class="composer">
        ...
        <button id="voiceBtn" title="Unirse a la sala de voz">🔊</button>
      </div>
    </section>
  </main>
</div>
...
<div id="settingsModal" class="modal"><div class="modal-body">
  ...
  <label>Notificaciones:
    <select id="notifSelect"><option value="sonido">Sonido</option><option value="vibracion">Vibración</option><option value="silencio">Silencio</option></select>
  </label>
  <label>Tema:
    <select id="themeSelect"><option value="oscuro">Oscuro</option><option value="claro">Claro</option><option value="montanas">Montañas</option><option value="playa">Playa</option><option value="corazones">Corazones</option></select>
  </label>
...
# --- frontend app.js (larger: servers/channels/messages/edit/delete/react/pin/notifications/local cache/jitsi) ---
...
function bindUI(){
  ...
  document.getElementById('fileInput').addEventListener('change', async e=>{
    ...
    const res = await fetch(API+`/channels/${currentChannel.id}/messages`,{method:'POST',body:fd})
